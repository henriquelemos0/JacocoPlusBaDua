...
public class CoverageMethodTransformer extends MethodTransformer {
	...
	public void transform(final MethodNode methodNode) {
		...
		final DefUseChain[] chains = DefUseChain.toBasicBlock(
				new DepthFirstDefUseChainSearch().search(frames, variables,
						successors, predecessors), leaders, basicBlocks);
		...
		for (int w = 0; w < windows; w++) {
			...
			LabelFrameNode.insertBefore(insn, methodNode.instructions,
					init(methodNode, w));
		}
		...
		for (int b = 0; b < basicBlocks.length; b++) {
			...
			final long[] lPotcovpuse = BitSetUtils.toLongArray(potcovpuse[b],
					windows);
			final long[] lBorn = BitSetUtils.toLongArray(born[b], windows);
			final long[] lDisabled = BitSetUtils.toLongArray(disabled[b],
					windows);
			...
			for (int w = 0; w < windows; w++) {
				...
				LabelFrameNode.insertBefore(first[b], methodNode.instructions,
						p);
			...
			if (isReturn(last[b].getOpcode())) {
				for (int w = 0; w < windows; w++) {
					...
					LabelFrameNode.insertBefore(last[b],
							methodNode.instructions, p);
				}
			}
		}
	...
	private Probe update(final MethodNode methodNode, final int window,
			final int index) {
		if (chains.length <= 32) {
